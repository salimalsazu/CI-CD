...
import { useDeleteCommentMutation } from "@/redux/features/blogs/commentApi";
...
const DeleteCommentConfirmationModal = ({
  ...
  const [deleteComment, { data, isSuccess, isError, isLoading, error, reset }] =
    useDeleteCommentMutation();

  const toaster = useToaster();

  const handleDeleteComment = async () => {
    await deleteComment({ commentId: deleteData?.commentId });
  };
  // ! side Effect
  useEffect(() => {
        ...
        <Modal.Body className="p-3 ">
          ...
          <div className=" ">
            ...
            <div className="mt-5 border p-5 border-black/10 rounded-lg">
              <h2>Category Name : {deleteData?.categoryName}</h2>
            </div>
            <div className="p-3  mt-5 bg-[#feeddd] rounded-lg ">
              ...
              <p className="text-[#b53a07] text-sm font-medium">
                By deleting this event , you will not be able to undo this
                action
              </p>
            </div>
