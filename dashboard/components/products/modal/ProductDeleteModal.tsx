...
const ProductDeleteModal = ({
  ...
  const [deleteProduct, { data, isSuccess, isError, isLoading, error, reset }] =
    useDeleteProductMutation();

  const toaster = useToaster();

  const handleDeleteProduct = async () => {
    await deleteProduct({ productId: deleteData?.productId });
  };
        ...
        <Modal.Body className="p-3 ">
          ...
          <div className=" ">
            ...
            <div className="mt-5 border p-5 border-black/10 rounded-lg">
              <h2>Product Name : {deleteData?.productName}</h2>
            </div>
            <div className="p-3  mt-5 bg-[#feeddd] rounded-lg ">
              ...
              <p className="text-[#b53a07] text-sm font-medium">
                By deleting this Product , you will not be able to undo this
                action
              </p>
            </div>
