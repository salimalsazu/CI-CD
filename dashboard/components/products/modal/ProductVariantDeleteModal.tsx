...
const ProductVariantDeleteModal = ({
  isOpenDelete,
  handleCloseDelete,
  deleteData,
}: any) => {
  ...
  const handleDeleteProductVariant = async () => {
    await deleteProductVariant({ variantId: deleteData?.variantId });
  };
        ...
        <Modal.Body className="p-3 ">
          ...
          <div className=" ">
            ...
            <div className="mt-5 border p-5 border-black/10 rounded-lg">
              <h2>Variant Name : {deleteData?.color?.name}</h2>
            </div>
            <div className="p-3  mt-5 bg-[#feeddd] rounded-lg ">
              ...
              <p className="text-[#b53a07] text-sm font-medium">
                By deleting this variant , you will not be able to undo this
                action
              </p>
            </div>
